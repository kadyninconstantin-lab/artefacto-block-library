...
(function () {
  ...
  // If today's occurrence hasn't passed yet → return today's date.
  // If it has already passed → return tomorrow's date.
  // DST-safe: uses calendar date arithmetic via Intl, never raw ±ms.
  function resolveDaily(timeStr, tzName) {
    var hm = (timeStr || '00:00').split(':');
    var h = +hm[0] || 0, m = +hm[1] || 0;

    // Get today's wall-clock date in the source timezone
    var now = new Date();
    var srcFmt = new Intl.DateTimeFormat('en-CA', { timeZone: tzName });
    var todayInSrc = srcFmt.format(now); // "YYYY-MM-DD"

    // Build candidate: today at HH:MM in tzName
    var pad = function(n) { return n < 10 ? '0' + n : '' + n; };
    var candidate = parseInTz(todayInSrc + 'T' + pad(h) + ':' + pad(m), tzName);

    // If candidate is in the past, advance by one calendar day in the source timezone
    if (candidate.getTime() <= now.getTime()) {
      // Get tomorrow's date in the source timezone by formatting candidate+25h
      // (+25h guarantees we cross midnight even during DST, then we re-snap to HH:MM)
  ...
  function renderEl(el) {
    ...
    try {
      var utcDate;
      if (dateStr === 'daily') {
        // Recurring daily mode — always show next upcoming occurrence
        var timeStr = el.getAttribute(ATTR_TIME) || '00:00';
        utcDate = resolveDaily(timeStr, tzName);
      } else {
